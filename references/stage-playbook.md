...
- **Дубль цифры/факта в близких местах.** Одна и та же запоминающаяся цифра, фамилия или цитата звучит дважды в соседних
  секциях — один раз вскользь по делу, второй раз на отдельном слайде-разрядке с подсветкой. На записи звучит как «ой,
  повторяюсь». Правило: **запоминающийся факт оставляем только в одном месте** — обычно на специальном слайде, где он
  визуально центрирован и работает как «момент истины». В исходном месте делаем **анонс с обещанием раскрыть**: «у
  Amazon частота деплоев измеряется не штуками в день, а интервалом между ними — конкретную цифру разберём через
  минуту». Это создаёт ожидание, и слайд срабатывает сильнее, чем повтор.
...
- **Generic stock imagery.** Photos that don't actually relate to the content. Either find specific imagery or use
  type-driven design.
- **«Одна цифра / одна строка в пустоте».** Зеркальная ошибка к walls of text: слайд с единственным элементом посередине
  пустого пространства смотрится как незаконченный draft. Лектор держит такой слайд 30-45 секунд под голос, и аудитория
  всё это время видит «одну цифру в пустоте». Правило: крупный элемент занимает 50-60% площади, остальное — вторичный
  контекст (источник, дата, расшифровка, тонкий визуальный якорь, callback). Подробнее — в style-guide, раздел
  «Плотность контента на слайде». Исключение — самый первый opening-слайд лекции, где минималистичность работает как
  «удар»; но даже там стоит дать опору взгляду мелким контекстом.
...
### Самопроверка на пересечения и user-visible дефекты

Обязательный шаг после первого рендера каждого слайда (и каждый раз после правок), до показа пользователю:

1. Сконвертируй `.pptx` в JPG (см. конвенцию в `anthropic-skills:pptx`).
2. Просмотри **каждый** слайд и явно проверь по чек-листу:
    - **Текст не перекрывает текст.** Заголовок не лезет на дугу/диаграмму, подписи к фигурам не сливаются с соседними,
      footer не упирается в основной контент.
    - **Текст не перекрывает фигуру и наоборот.** Стрелка не идёт сквозь слово, кружок-маркер не закрывает буквы,
      линия-разделитель не накладывается на нижнюю кромку букв.
    - **Текст влезает в свой контейнер.** Длинная цифра не переползает на две строки. Текст в круге не разрывается на
      «СМУЗ»+«И». Главная мысль не выпадает за пределы слайда.
    - **Стрелка указывает туда, куда нужно по смыслу.** Если ops «держит» серверы — стрелка от ops к серверам, а не
      наоборот. Это самая частая методологическая ошибка, которая не отлавливается визуально без проверки смысла.
    - **Порядок элементов соответствует смыслу.** Цепочка «X → Y → Z» должна читаться слева направо и совпадать с
      реальной логикой процесса (например, dev → стена → ops → серверы, а не dev → серверы → ops).
    - **Эмодзи отрендерились.** Если в pptx был emoji-символ и в JPG вместо него квадрат-плейсхолдер — заменяй на
      shapes-иконку или текстовую метку. LibreOffice без emoji-fonts молча роняет emoji.
3. Если нашёл хотя бы один дефект — фикси и **рендери заново**. Не показывай пользователю слайд с пересечением текста
   или стрелкой не туда — это съедает доверие к стилю и каждый дефект, оставленный на ревью, превращается в круг правок.
4. После одного fix-and-verify цикла остановись (по правилу из `pptx` skill). Дальнейшие правки — по комментариям
   пользователя.

**Самые частые user-visible дефекты, по которым стоит проверять явно:**

- крупная цифра/заголовок переползает на две строки и закрывает контекст под ней;
- decorative line / divider под главной мыслью накладывается на нижнюю кромку букв;
- стрелка между фигурами идёт «навстречу» (наконечник не там, где смысл);
- порядок элементов в композиции противоречит логике процесса;
- emoji в pptx → квадрат в JPG (LibreOffice).
