...
- **Тире vs дефис при заменах.** Клаузальное тире « — » — с пробелами с обеих сторон; дефис в составных словах и URL —
  без пробелов («что-то», «из-за», `service-accounts`). Глобальная замена «—»↔«-» ломает составные слова и ссылки —
  заменяй только тире, окружённое пробелами; внутрисловные дефисы не трогай.
...
### Примеры без реальных публичных лиц

В примерах не привязывайся к реальным публичным лицам и их историям (новостной кейс, скандал, конкретный человек): это
этически скользко и быстро устаревает. Давай механику обезличенно — узнаваемость несёт сам механизм, а не имя. (Связано
с правилом про городские легенды в разделе Fact-check.)

### Финальная корректура: патч + поштучный ревью

Перед записью/сдачей прогони корректуру русского (пунктуация, орфография, типографика) и оформи правки в **патч-файл**,
по которому пройдись с автором по одной правке в формате квиза («принять / нет»). Применяй только одобренные — так автор
сохраняет контроль над голосом.

### Типичные замечания ревью (анти-паттерны)

Собрано из реальных ревью на «плохом» материале — частые «упавшие тесты». Проверяй и по ним:

- **Самодостаточная отсылка.** Callback к пройденному должен нести смысл сам по себе, а не требовать «вспомни, что мы
  говорили» — иначе слушатель застревает.
- **Одно простое определение.** Даёшь одно определение, не академичное; не ставишь рядом два определения одного и того
  же.
- **Нет неразъяснённых терминов/концептов.** Ввёл — сразу «что это и зачем здесь» (см. также gloss англицизмов).
- **Вынос за скоуп объяснён.** Не «об этом не говорим», а одной фразой почему + куда смотреть.
- **Нет «почитать» без ссылки.** Любая отсылка к источнику — с рабочей ссылкой.
...
### Плотность контента на слайде

**Не делай слайдов «одна цифра в пустоте» или «одна строка крупно — и всё».** Минималистичный слайд с единственным
элементом, занимающим 90% площади, выглядит как незаконченный draft, а не как сильное визуальное высказывание. На записи
аудитория ловит «и это всё, что у вас есть на этом слайде?» — а лектор всё равно держит слайд 30-45 секунд, пока
разворачивает мысль голосом.

**Что делать вместо.** Если нужен крупный акцент (большая цифра, ключевая фраза, цитата) — окружи его **вторичным
контекстом**, который дополняет, но не отнимает внимание:

- **Большая цифра** → крупно цифра **+** ниже мелким **что это**: источник, дата, единица измерения, расшифровка.
    - Плохо: «$440 000 000» в центре пустого чёрного слайда.
    - Хорошо: «$440 000 000» крупно + ниже мелким «Knight Capital · 1 августа 2012 · 45 минут торговли».
- **Главная мысль секции** → формулировка **+** один из: тонкий визуальный якорь (иконка по теме секции), горизонтальный
  разделитель с маркером секции, callback-ссылка на ключевой образ.
- **Quote** → цитата **+** атрибуция **+** (опционально) короткая ссылка-связка с тезисом лекции.
- **Cover-слайд (opening/closing)** → ключевой образ/цифра **+** заголовок секции/даты/контекст.

**Размер крупного элемента — 50-60% площади**, не 90%. Остальное пространство — для контекстных элементов, дыхания
композиции, footer-брендинга, номера слайда.

**Критерий проверки.** Покажи слайд на 5 секунд человеку без речитатива и спроси: «О чём это?» Если ответ — «не знаю,
тут только цифра/слово/цитата без контекста», добавляй вторичный контекстный элемент.

Исключение: **самый первый слайд лекции** (cover-opening, который держится 30+ секунд и работает как «удар»). Здесь
допустимо более минималистично, но даже там стоит мелким контекстом (дата, локация) дать опору взгляду на 2-3 секунде
просмотра. Без этого слайд воспринимается как «ошибка генерации презентации».

### Тематические плашки (Sec-плашки и аналогичные) — не текст, а визуал

Слайд с цветной плашкой (Sec-плашка, Knight-привязка, любая другая категорийная маркировка с эмодзи в заголовке) — это
**сквозной визуальный мотив**, и наполнение должно быть визуальным, не текстовым. Три строки тезисов на цветной плашке
выглядят как чек-лист на корпоративном интранете: глаз скользит мимо, лектор всё равно проговаривает голосом, слайд не
работает.

**Правило.** Плашка-шапка (цветная полоса + иконка + тематический заголовок) остаётся как маркер категории — узнаваемый
шаблон, общий для всех слайдов этого типа в лекции. **Под шапкой** должен быть содержательный визуал по теме плашки:

- Сравнительная диаграмма («расходящиеся ножницы» — две стрелки с разной динамикой, маркер ключевого года).
- Схема процесса/pipeline с подсвеченными точками.
- Сетка иконок-практик (SAST + secret scanning + dependency scanning как три иконки в ряд, не три строки текста).
- Графический контраст «было vs стало» / «без vs с».

Если визуал по теме плашки не находится — проверь, нужна ли тут вообще плашка. Возможно, тематика лучше отрабатывается
короткой ремаркой в речитативе на обычном content-слайде, без сквозного маркера.

**Антипример (так не делать):**

> Жёлтая шапка «🔒 А что с безопасностью?» сверху,
> под ней три строки текста:
>  • «Скорость выросла»
>  • «Безопасность осталась последним шагом»
>  • «С 2012 года стал формироваться DevSecOps».

**Хорошо:**

> Жёлтая шапка «🔒 А что с безопасностью?» сверху, под ней — диаграмма «расходящиеся ножницы»: зелёная стрелка
> «скорость доставки» растёт с 2008-го, серая стрелка «security review» плоская, красный пунктир и точка пересечения
> на 2012-м с подписью «здесь появился DevSecOps». Внизу одна short summary-строка.
